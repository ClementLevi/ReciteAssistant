...
class Spliter:
    ...
    def _chiSplit(self):
        ...
        # def clearSpace():
            # for each in self.text_output: # 清除中英文混杂时的英文空格，默认关闭（因为要按原样打印文本）
                # if each == ' ':
                    # self.text_output.remove(each)
        # clearSpace()
    ...
    def _latinSplit(self):
        ...
        # index = len(self.text_output)
        # for _ in range(len(self.text_output)): # 恢复了空格，否则输出时会连成一条
        #     self.text_output.insert(index, ' ')
        #     index -= 1
        # 果然还是不分开了比较方便处理。输出的时候根据文本类型添加空格
        return self.text_output
...
class TestGiver(Article):
    ...
    def new_test(self, difficulty=3):
        ...
        # # 检测文本语言类型，若非汉语则打印输出时每个字符后加空格
        # if self.Article.basic["text_type"] != "Chinese":
        #     exam = westoutput(possibility_distributed)
        # else:
        #     exam = chioutput(possibility_distributed)
...
if __name__ == "__main__":
    ...
    # a = Spliter(target_text='Deutsche über alles. Wir sind „Was ist das“?')
    # a = Spliter(target_text='.。。。。。。。。。。。。。。。')
    # a = Spliter(target_text='韩国人：洗剪吹 \
    # \
    # 中国人、意大利人：餐馆 ')
    # print("text digest result: ", a.text_output)
    # print("text type: ", a.text_type, '\n')

    # b = Article()
    # print(b.dumps(2))
    # b.text_digested = ['There', ' ', 'will', ' ', 'be', ' ', 'blood', '.']
    # print(b.text_content())

    # b = Article()
    # b.loadf('./Article_data.json')
    # print(b.content["2"]['Fw'])
    # b.show_content(2)
    # b.show_basic_info(3)
